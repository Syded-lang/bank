...
class ManageUsersController extends Controller
{
    ...
    public function create()
    {
        ...
        $branches = \App\Models\Branch::where('status', Status::ENABLE)->orderBy('name')->get();
        return view('admin.users.create', compact('pageTitle', 'branches'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'username' => 'required|string|max:40|unique:users|alpha_dash',
        ...
        // Set account as active and verified by default (admin created)
        ...
        // Create transaction record if initial balance was added
        if ($initialBalance > 0) {
            $transaction = new Transaction();
            $transaction->user_id = $user->id;
            $transaction->amount = $initialBalance;
            $transaction->post_balance = $initialBalance;
            $transaction->charge = 0;
            $transaction->trx_type = '+';
            $transaction->remark = 'initial_balance';
            $transaction->details = 'Initial balance added by admin';
