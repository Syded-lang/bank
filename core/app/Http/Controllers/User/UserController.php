...
class UserController extends Controller {
    ...
    public function update2faMethod(Request $request) {
        $request->validate([
            'preferred_2fa_method' => 'required|in:google,email,sms',
        ]);

        $user = auth()->user();
        
        // Check if Google Authenticator is selected but not enabled
        if ($request->preferred_2fa_method == 'google' && !$user->ts) {
            $notify[] = ['error', 'Please enable Google Authenticator first before selecting it as your preferred method'];
            return back()->withNotify($notify);
        }
        
        // Check if Email OTP is enabled in system
        if ($request->preferred_2fa_method == 'email' && !gs()->modules->otp_email) {
            $notify[] = ['error', 'Email OTP is not enabled in the system'];
            return back()->withNotify($notify);
        }
        
        // Check if SMS OTP is enabled in system  
        if ($request->preferred_2fa_method == 'sms' && !gs()->modules->otp_sms) {
            $notify[] = ['error', 'SMS OTP is not enabled in the system'];
            return back()->withNotify($notify);
        }

        $user->preferred_2fa_method = $request->preferred_2fa_method;
        $user->save();
        ...
        $notify[] = ['success', 'Your preferred 2FA method has been updated to ' . $methodNames[$request->preferred_2fa_method]];
        return back()->withNotify($notify);
    }
